"""US-PRE-E00-S03 — LLM inference contract, exercised against a fake LLM
(no GPU/model required): temperature=0 + seed on every call, JSON-only
system prompt, one repair attempt, LLM_OUTPUT_INVALID if still broken.
"""
import json
...
def test_invalid_then_valid_repairs_on_second_attempt(monkeypatch):
    ...
    # repair attempt must still use temperature=0 and the same seed
